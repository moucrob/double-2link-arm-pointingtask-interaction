...
dimsArms = [ones(2,1)*0.5 ones(2,1)*0.5 ones(2,1)*0.075 ones(2,1)*0.06]; %(m) (logic below)
...
scal = 30; %%% TUNABLE %%% length of the two side parts of the arrow heads
...
c2_1 = dimsArms(1,4); %width of the squared base of the cylinder (1st arm)
a1_2 = dimsArms(2,1);
a2_2 = dimsArms(2,2);
b2_2 = dimsArms(2,3); 
c2_2 = dimsArms(2,4);
...
templateForeV = [x1,y1,z1];
toBeTransformed = [templateForeV';ones(1,size(V1,1))];
...
%         posJoints1(i,:) = [posStart1 something posEnd1];
        ...
        %Now I have to "represent" my foreArm cubes by using t3x1 as Elb1or2 (2x1 only):
        toTrim1 = T(sum(Q1(i,:)),[Elb1(i,:),0])*toBeTransformed;
        V1{i,1} = toTrim1(1:3,:)';
        toTrim2 = T(sum(Q2(i,:)),[Elb2(i,:),0])*toBeTransformed;
        V2{i,1} = toTrim2(1:3,:)';
...
%             disp(['length fore2 = ',num2str(norm(Elb2(j,:)-P2(j,:)))])
        ...
        delete(angle1_p1);
        angle1_p1 = plot_arc(0,Q1(j,1),orig1,r,'g',scal);
        delete(angle2_p1);
        angle2_p1 = plot_arc(Q1(j,1),sum(Q1(j,:)),Elb1(j,:),r,'g',scal);
        delete(angle1_p2);
        angle1_p2 = plot_arc(0,Q2(j,1),orig2,r,'b',scal);
        delete(angle2_p2);
        angle2_p2 = plot_arc(Q2(j,1),sum(Q2(j,:)),Elb2(j,:),r,'b',scal);
